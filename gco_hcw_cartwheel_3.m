...
% consider a Cartwheel formation. Two Chasers. One Leader that is
% imaginary. and is located at the centre of the chasers
% This leader reference coincides with the origin of the LVLH frame.
% 
...
% --mean motion
t = 0 ; %seconds
alpha = deg2rad(0+[0 120 240].'); % radians, 0 +90  deg for the two phases each time
...
n = sqrt(mu/(x_leader^3)); % mean motion of leader satellite (reference orbit)
...
%x_0 = (-2 *c1) * sin(n*t + alpha); % metres in LVLH-x
x_0 = (c1/2) * sin(n*t + alpha);
x_c = x_leader + x_0; %metres in ECI-x

y_0 = (c1) * cos( n*t + alpha ) + c3  ; % it is 0 metres in LVLH-y
y_c = y_0 + y_leader ; %metres in ECI-y

%z_0 = c2 * cos(n*t - alpha ) ; % metres in LVLH-z
z_0 = c2 * sin(n*t + alpha ) ;
...
%xdot_0 = (-2 * c1) * n * cos (n*t + alpha); % metres/second in LVLH-x
xdot_0 = (c1/2) * n * cos (alpha);
xdot_c = xdot_leader + xdot_0 + xdot_rotation ; % metres/second in ECI-x
disp('xdot_c');
disp(xdot_c);

ydot_0 = (-c1) * n * sin(alpha);
ydot_c = ydot_leader + ydot_0 + ydot_rotation ; % metres/second in ECI-y
disp('ydot_c');
disp(ydot_c);

%zdot_0 = -c2 * n * sin(n*t - alpha); % metres/second in LVLH-z
zdot_0 = c2 * n * cos(alpha);
