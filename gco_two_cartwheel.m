...
% --mean motion
t = 0 ; %seconds
alpha = deg2rad(0+[0 180].'); % radians, 0 +180  deg for the two phases each time
...
y_0 = (0)*cos( n*t + alpha ) + c3 ; % it is 0 metres in LVLH-y
y_c = y_0 + y_leader ; %metres in ECI-y

z_0 = c1 * cos(n*t - alpha ) ; % metres in LVLH-z
...
zdot_0 = -c1 * n * sin(n*t - alpha); % metres/second in LVLH-z
