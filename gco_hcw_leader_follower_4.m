...
% The below code calculates the initial position and velocity of chaser
...
% --mean motion
t = 0 ; %seconds
alpha = deg2rad([180 180 180].'); % radians
...
xdot_0 = (c1/2) * (n) * cos (n*t + alpha); % metres/second in LVLH-x
xdot_c = xdot_leader + xdot_0 + xdot_rotation ; % metres/second in ECI-x
disp('xdot_c');
disp(xdot_c);

ydot_0 = (-c1) * n * sin(n*t + alpha); % metres/second in LVLH-y
ydot_c = ydot_leader + ydot_0 + ydot_rotation ; % metres/second in ECI-y
disp('ydot_c');
disp(ydot_c);

zdot_0 = c2 * n * cos(n*t + alpha); % metres/second in LVLH-z
