...
% consider a Square formation. Four Chasers. One Leader that is
% imaginary. and is located at the centre of the square
% formed by the Four satellites. This centroid coincides with the origin
% of the LVLH frame.
...
%side = 100 ; % metres , side of the equilateral Pentagon

% rho = Relative distance between the centroid of the Square and the 
% --respective deputies in metres deputies are in vertex
rho = side*sqrt(2)/2; % metres
...
% --mean motion
t = 0 ; %seconds
alpha = deg2rad(9+[0 90 180 270].'); % radians, 9 +90  deg for the five phases each time
...
rotation_term = cross(repmat(theta_dot.',4,1),rho_eci,2); % we get the rotation translation matrix
