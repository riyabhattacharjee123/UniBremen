Nr = 64;
k = 1:100 ;
r1= r_start_sat_gs_spherical(1,1); %8.897389705668777e+06 % 8.897714448631750e+06; %9.554100016146174e+06;%meters 
r2= r_start_sat_gs_spherical(1,4);

theta_inc_l=[];
theta_inc_j=[];
theta_inc_i = r_start_sat_gs_spherical(1,2);
...
phi_i = r_start_sat_gs_spherical(1,3); %-0.681495497830870 %0.542473753252491; %-0.963735944517760 ; % radians
phi_l=[];
phi_j=[];
...
for ck=1:length(k)    
    if not(mod(ck,Nrx)== 0)
        ...
        phi_l(1,ck) = acos(cos(phi_i)+(2*ck/Nrx));
        phi_j(1,ck) = acos(cos(phi_l(1,ck))+(2*ck/Nrx));
