#define S_FUNCTION_NAME  air_drag_force_s_function
...
static void mdlInitializeSizes(SimStruct *S)
{
   ...
   // PORT 2: SATELLITE VELOCITY
    ...
    // One outputs with tone entry each: C_D and C_L
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    double param_arr_pos_in[3], param_arr_vel_in[3], param_arr_out[3];
  
    // Get parameters from inputs
  real_T *sat_position = (real_T *) ssGetInputPortRealSignal(S, 0);
  real_T *sat_velocity = (real_T *) ssGetInputPortRealSignal(S, 1);
  ...
  // Define output variable  
    real_T *new_acc_drag = (real_T *) ssGetOutputPortRealSignal(S, 0);


    //double output[3];
    for (i=1; i<= n_sat; i++)
    {
        param_arr_pos_in[0] = sat_position[3*i-3];
        param_arr_pos_in[1] = sat_position[3*i-2];
        param_arr_pos_in[2] = sat_position[3*i-1];        

        param_arr_vel_in[0] = sat_velocity[3*i-3];
        param_arr_vel_in[1] = sat_velocity[3*i-2];
        param_arr_vel_in[2] = sat_velocity[3*i-1];        

        // Call c-fucntion
        
        air_drag(param_arr_pos_in,param_arr_vel_in, param_arr_out);
        
        // Return calculated acceleration vector
   
        new_acc_drag[3*i-3] = param_arr_out[0];
        new_acc_drag[3*i-2] = param_arr_out[1];
        new_acc_drag[3*i-1] = param_arr_out[2];
    }
}
