...
{   
    ...
    double val = pi/180;
    double rho_sun_sat[3],sun_cood[3], part_one[3], part_two[3];    
    ...
    //inclination of ecliptic relative to earth's equator
    ...
    // magnitude of the Sun coods, cubed.
    double dr_b = pow((pow(sun_cood[0],2) + pow(sun_cood[1],2)\
            + pow(sun_cood[2],2)),1.5);
    // Distance between Sun and satellite in ECI   
    rho_sun_sat[0] = sun_cood[0] - pos[0];
    rho_sun_sat[1] = sun_cood[1] - pos[1];
    rho_sun_sat[2] = sun_cood[2] - pos[2];
    // magnitude of the distance between Sun and satellite cubed
    double dr_a = pow((pow(rho_sun_sat[0],2) + pow(rho_sun_sat[1],2)\
            + pow(rho_sun_sat[2],2)),1.5);    
