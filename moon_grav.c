/* ------------------------------------------------------------------ */
/* Calculate perturbation acceleration due to Gravitation of the Moon */
/* ------------------------------------------------------------------ */
...
{   
    ...
    double rho_moon_sat[3], moon_cood[3], part_one[3], part_two[3];
    double val = pi/180;
    ...
    double beta = (18520/3600)*\
            sin(val*(F+lambda-L_0+(412/3600)*sin(2*F*val)\
            +(541/3600)*sin(l_sun*val)))\
    ...
    double epsilon = 23.0+(26.0/60.0)+ (21.448/3600.0)\
            - (46.8150*T+ 0.00059*T*T- 0.001813*T*T*T)/3600; // degree
    // Moon's distance from the center of the Earth
    double rho_m = 385000 - 20905*cos(l*val) - 3699* cos((2*D-l)*val)\
            -2956 * cos(2*D*val) - 570 * cos(2*l*val)\
            + 246 * cos(2*(l-D)*val)-205*cos((l_sun-2*D)*val)\
            - 171 * cos((l+2*D)*val) - 152*cos((l+l_sun-2*D)*val); // Km
    double rho_moon = 1000*rho_m ; // m
    // calculating Lunar Coordinates in ECI
    moon_cood[0] = rho_moon*cos(lambda*val)*cos(beta*val);
    moon_cood[1] = rho_moon*(sin(lambda*val)*cos(beta*val)\
            *cos(epsilon*val)-sin(epsilon*val)*sin(beta*val));
    moon_cood[2] = rho_moon*(sin(lambda*val)*cos(beta*val)\
            *sin(epsilon*val)+cos(epsilon*val)*sin(beta*val));
    // The magnitude of Moon coordinates, cubed
    double dr_b = pow((pow(moon_cood[0],2)+pow(moon_cood[1],2)\
            +pow(moon_cood[2],2)),1.5);
    // Distance between Moon and satellite in ECI
    rho_moon_sat[0] =  moon_cood[0] - pos[0];
    rho_moon_sat[1] =  moon_cood[1] - pos[1];
    rho_moon_sat[2] =  moon_cood[2] - pos[2];
    // Magnitude of the Moon satellite distance, cubed
    double dr_a = pow((pow(rho_moon_sat[0],2) + pow(rho_moon_sat[1],2)\
            + pow(rho_moon_sat[2],2)),1.5);
